...
class EnhancedFaceDetector:
    def __init__(self, data_dir=None, models_dir=None):
        ...
        self.emotion_feedback = {
            ...
            "sad": [
                ...
                "Consider doing something you enjoy today.",
                "Talking to someone might help lift your spirits."
            ],
            "angry": [
                "Taking deep breaths can help manage anger.",
                "Consider stepping away for a moment to cool down.",
                "Try to identify what's triggering your frustration.",
            ...
            "surprise": [
                "Something unexpected caught your attention!",
                "Surprises keep life interesting!",
                "Your curious expression shows engagement.",
                "What an interesting reaction!"
            ],
            "disgust": [
                "Something seems to be bothering you.",
                "It's okay to remove yourself from uncomfortable situations.",
                "Your feelings are valid.",
                "Consider what might help you feel more at ease."
            ]
        }
        
        # Stress relief suggestions based on detected stress indicators
        self.stress_relief_suggestions = {
            "eye strain": [
                "Try the 20-20-20 rule: Every 20 minutes, look at something 20 feet away for 20 seconds.",
                "Consider adjusting your screen brightness or using blue light filters.",
                "Gently massage around your eyes to relieve tension.",
                "Ensure proper lighting in your environment to reduce eye strain."
            ],
            "jaw tension": [
                "Practice jaw relaxation exercises: gently open and close your mouth several times.",
                "Apply a warm compress to your jaw area for 5-10 minutes.",
                "Be mindful of teeth clenching or grinding, especially during concentration.",
                "Try gentle jaw stretches by slowly moving your jaw from side to side."
            ],
            "frowning": [
                "Take a moment to relax your forehead muscles by gently massaging them.",
                "Practice mindfulness to become aware of when you're frowning.",
                "Try facial relaxation exercises by raising and lowering your eyebrows slowly.",
                "Consider what might be causing stress and address the root cause if possible."
            ]
        }
        
        # General stress relief suggestions
        self.general_stress_relief = [
            "Take a few deep breaths, inhaling for 4 counts and exhaling for 6.",
            "Consider a short break to stretch or walk around.",
            "Hydrate yourself - sometimes stress symptoms increase with dehydration.",
            "Practice progressive muscle relaxation by tensing and releasing muscle groups.",
            "Try to maintain good posture to reduce physical tension."
        ]
    ...
    def get_assistant_response(self, emotion, age_range, gender, head_pose=None, facial_structure=None, stress_indicators=None):
        """
        ...
        Args:
            ...
            stress_indicators: Detected stress indicators (if available)
        ...
        try:
            ...
            stress_info = ""
            if stress_indicators and stress_indicators["indicators"]:
                stress_info = f" Stress indicators detected: {', '.join(stress_indicators['indicators'])}."
                
            prompt = f"""The camera sees a {gender} person in the age range {age_range} who appears to be {emotion}.{pose_info}{facial_info}{stress_info}

Based on their facial structure analysis and detected attributes, provide:
1. A personalized haircut recommendation that considers their facial proportions, gender, and age.
2. If stress indicators are present, suggest 1-2 specific techniques to address them.

Keep your response brief (3-4 sentences) and conversational."""
            ...
            # Add haircut recommendation if facial structure is available
            if facial_structure and "analysis" in facial_structure:
                ...
                if face_shape in self.haircut_recommendations and gender in self.haircut_recommendations[face_shape]:
                    response += self.haircut_recommendations[face_shape][gender] + " "
            else:
                response += "Consider a hairstyle that balances your facial proportions. "
            
            # Add stress relief suggestions if stress indicators are available
            if stress_indicators and stress_indicators["indicators"]:
                if stress_indicators["level"] == "high":
                    response += "I notice signs of stress. "
                    
                    # Add specific suggestions for detected indicators
                    for indicator in stress_indicators["indicators"]:
                        indicator_key = next((k for k in self.stress_relief_suggestions.keys() if k in indicator), None)
                        if indicator_key:
                            response += np.random.choice(self.stress_relief_suggestions[indicator_key]) + " "
                            break
                    
                    # Add a general stress relief suggestion
                    response += np.random.choice(self.general_stress_relief)
                else:
                    response += np.random.choice(self.general_stress_relief)
    ...
    def get_stress_relief_suggestion(self, stress_indicators):
        """
        Get personalized stress relief suggestions based on detected stress indicators
        
        Args:
            stress_indicators: Dictionary containing detected stress indicators
            
        Returns:
            Stress relief suggestion as string
        """
        if not stress_indicators or not stress_indicators["indicators"]:
            return np.random.choice(self.general_stress_relief)
        
        suggestions = []
        
        # Add specific suggestions for each detected indicator
        for indicator in stress_indicators["indicators"]:
            indicator_key = next((k for k in self.stress_relief_suggestions.keys() if k in indicator), None)
            if indicator_key:
                suggestions.append(np.random.choice(self.stress_relief_suggestions[indicator_key]))
        
        # Add a general suggestion if no specific ones were found
        if not suggestions:
            suggestions.append(np.random.choice(self.general_stress_relief))
        
        # Return a random suggestion from the collected ones
    ...
    def detect_stress_indicators(self, face_img, landmarks):
        """
        Detect stress indicators based on facial landmarks
        ...
        Returns:
            Dictionary containing stress indicator analysis
        """
        if not landmarks:
            return None
        ...
        # Eye landmarks (left eye: 159, 145, 33, 133; right eye: 386, 374, 263, 362)
        ...
        # Calculate eye openness (vertical distance between top and bottom landmarks)
        ...
        # Eye width for reference (to normalize openness)
        left_eye_width = np.linalg.norm(np.array(landmark_points[33]) - np.array(landmark_points[133]))
        right_eye_width = np.linalg.norm(np.array(landmark_points[263]) - np.array(landmark_points[362]))
        avg_eye_width = (left_eye_width + right_eye_width) / 2
        
        # Calculate eye openness ratio
        eye_ratio = avg_eye_openness / avg_eye_width if avg_eye_width > 0 else 0
        
        # Check for eye strain (squinting)
        if eye_ratio < 0.2:  # Threshold for squinting
            stress_indicators.append("eye strain (squinting)")
            stress_score += 1
        
        # Check for jaw tension
        # Jaw landmarks (left: 172, right: 397, bottom: 152)
        jaw_left = landmark_points[172]
        jaw_right = landmark_points[397]
        jaw_bottom = landmark_points[152]
        
        # Calculate jaw angle
        jaw_width = np.linalg.norm(np.array(jaw_left) - np.array(jaw_right))
        jaw_height = np.linalg.norm(np.array(jaw_bottom) - (np.array(jaw_left) + np.array(jaw_right)) / 2)
        jaw_ratio = jaw_height / jaw_width if jaw_width > 0 else 0
        
        # Check for jaw tension (clenched jaw has lower height-to-width ratio)
        if jaw_ratio < 0.3:  # Threshold for jaw tension
            stress_indicators.append("jaw tension")
            stress_score += 1
        
        # Check for frowning
        # Eyebrow landmarks (left: 65, 105; right: 295, 334)
        left_eyebrow_inner = landmark_points[65]
        left_eyebrow_outer = landmark_points[105]
        right_eyebrow_inner = landmark_points[295]
        right_eyebrow_outer = landmark_points[334]
        
        # Calculate eyebrow angle
        left_eyebrow_angle = np.degrees(np.arctan2(
            left_eyebrow_inner[1] - left_eyebrow_outer[1],
            left_eyebrow_inner[0] - left_eyebrow_outer[0]
        ))
        right_eyebrow_angle = np.degrees(np.arctan2(
            right_eyebrow_inner[1] - right_eyebrow_outer[1],
            right_eyebrow_outer[0] - right_eyebrow_inner[0]
        ))
        
        # Check for frowning (negative angles indicate downward inner eyebrows)
        if left_eyebrow_angle < -15 or right_eyebrow_angle < -15:
            stress_indicators.append("frowning")
            stress_score += 1
    ...
    def detect_faces(self, img):
        ...
        for (x, y, w, h) in faces:
            ...
            if results and results.multi_face_landmarks:
                ...
                stress_indicators = self.detect_stress_indicators(img[y:y+h, x:x+w], results.multi_face_landmarks[0])
                if stress_indicators and stress_indicators["indicators"]:
                    stress_text = f"Stress: {stress_indicators['level'].capitalize()} ({', '.join(stress_indicators['indicators'])})"
            
            # Estimate head pose
            head_pose = self.estimate_head_pose(img[y:y+h, x:x+w], (x, y, w, h))
            
            # Use DeepFace for advanced emotion recognition
            try:
                # Extract face for DeepFace analysis
                ...
                # Format emotion confidence scores
                emotion_scores = []
                for emotion, score in emotions.items():
                    emotion_scores.append(f"{emotion}: {score:.1f}%")
                
                # Create emotion text display
                emotion_text = f"Emotion: {dominant_emotion.capitalize()}"
                
                # Get emotion feedback
                emotion_feedback = self.get_emotion_feedback(dominant_emotion)
            ...
            # Get stress relief suggestion if stress indicators are detected
            stress_suggestion = ""
            if stress_indicators and stress_indicators["indicators"]:
                stress_suggestion = self.get_stress_relief_suggestion(stress_indicators)
            ...
            if self.age_net is not None and self.gender_net is not None:
                ...
                if self.assistant_enabled:
                    ...
                    assistant_response = self.get_assistant_response(
                        ...
                        gender, 
                        head_pose, 
                        facial_structure_desc, 
                        stress_indicators
                    )
                    
                    # Display assistant response
                    cv2.putText(img, "Recommendations:", (10, img.shape[0] - 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            ...
            # Display emotion, age, gender, head pose, facial structure, and stress indicators
            cv2.putText(img, emotion_text, (x, y-40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            
            # Display emotion scores
            for i, score_text in enumerate(emotion_scores[:3]):  # Show top 3 emotions
                cv2.putText(img, score_text, (x, y+h+20+i*20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
            ...
            if stress_text:
                cv2.putText(img, stress_text, (x, y-120), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            
            # Display emotion feedback and stress suggestion in the top-left corner
            if emotion_feedback:
                cv2.putText(img, f"Feedback: {emotion_feedback}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
            if stress_suggestion:
                # Split long stress suggestion into multiple lines
                words = stress_suggestion.split()
                lines = []
                current_line = "Suggestion: "
                
                for word in words:
                    if len(current_line + " " + word) <= 60:  # Limit line length
                        current_line += " " + word if "Suggestion: " not in current_line or current_line != "Suggestion: " else word
                    else:
                        lines.append(current_line)
                ...
                for i, line in enumerate(lines):
                    cv2.putText(img, line, (10, 90 + i*25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
